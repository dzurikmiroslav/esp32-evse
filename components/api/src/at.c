...
{
    echo_flag = (cmd->name[strlen(cmd->name) - 1] == '1') ? 1 : 0;
...
{
    serial_id_t id = cmd->name[strlen(cmd->name) - 1] - '1';
    var_u8_1 = 5 + serial_get_data_bits(id);
...
{
    serial_id_t id = cmd->name[strlen(cmd->name) - 1] - '1';
    var_u8_2 = 1 + serial_get_stop_bits(id);
...
static cat_return_state cmd_serial_config_write(const struct cat_command* cmd, const uint8_t* data, const size_t data_size, const size_t args_num)
{
    ...
    uart_word_length_t data_bits = var_u8_1 - 5;
    uart_stop_bits_t stop_bits = var_u8_2 - 1;
    ...
    return serial_set_config(id, baud_rate, data_bits, stop_bits, parity) == ESP_OK ? CAT_RETURN_STATE_OK : CAT_RETURN_STATE_ERROR;
}
...
static int var_serial_mode_read(const struct cat_variable* var, const struct cat_command* cmd)
{
    serial_id_t id = cmd->name[strlen(cmd->name) - 1] - '1';
...
static int var_serial_mode_write(const struct cat_variable* var, const struct cat_command* cmd, const size_t write_size)
{
    serial_id_t id = cmd->name[strlen(cmd->name) - 1] - '1';
    serial_mode_t mode = var_u8_1;

    return serial_set_mode(id, mode) != ESP_OK;
}

static struct cat_variable vars_serial_mode[] = {
    {
        .type = CAT_VAR_UINT_DEC,
        .data = &var_u8_1,
        .data_size = sizeof(var_u8_1),
        .name = "mode",
        .access = CAT_VAR_ACCESS_READ_WRITE,
        .read_ex = var_serial_mode_read,
        .write_ex = var_serial_mode_write
    }
};
...
static struct cat_command cmds[] = {
    ...
    {
        .name = "+SRLMODE1",
        .var = vars_serial_mode,
        .var_num = sizeof(vars_serial_mode) / sizeof(vars_serial_mode[0]),
        .need_all_vars = true
    },
    {
        .name = "+SRLMODE2",
