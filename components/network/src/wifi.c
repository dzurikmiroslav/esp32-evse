...
#define NR_IE_MIN_LEN (ETH_ALEN + 4 + 1 + 1 + 1)
    ...
    while (report_len >= 2 + NR_IE_MIN_LEN) {
        ...
        if (pos[0] != WLAN_EID_NEIGHBOR_REPORT || nr_len < NR_IE_MIN_LEN) {
            ESP_LOGI(TAG, "CTRL: Invalid Neighbor Report element: id=%u len=%u", data[0], nr_len);
            ret = -1;
            goto cleanup;
        }

        if (2U + nr_len > report_len) {
            ESP_LOGI(TAG, "CTRL: Invalid Neighbor Report element: id=%u len=%zu nr_len=%u", data[0], report_len, nr_len);
            ret = -1;
            goto cleanup;
        }
        pos += 2;
        ...
        ESP_LOGI(TAG,
                 "RMM neighbor report bssid=" MACSTR " info=0x%" PRIx32 " op_class=%u chan=%u phy_type=%u%s%s%s%s",
                 MAC2STR(nr),
...
cleanup:
    if (neighbor_list) free(neighbor_list);
}
...
void wifi_init(void)
{
    ...
    ESP_ERROR_CHECK(esp_event_handler_register(WIFI_EVENT, WIFI_EVENT_STA_NEIGHBOR_REP, &neighbor_report_recv_handler, NULL));
